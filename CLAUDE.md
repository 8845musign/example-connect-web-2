...
1. **「正しく」という曖昧な表現を避ける**
   - ❌ `正しく更新する`
   - ✅ `isConnectedがtrue、currentUserIdとusersが設定され、errorがクリアされる`

2. **具体的な期待値を明示する**
   - アクションと結果の関係を明確に記述
   - 何が変更され、何が保持されるかを明確にする

3. **振る舞いベースの命名**
   - 実装の詳細ではなく、ビジネスロジックの観点から命名
   - Given-When-Then形式を意識する
...
4. **例**
   ...
   // ✅ 良い例
   test("USER_JOINED actionで新規ユーザーが既存リストに追加される", () => {});
   
   // ✅ エッジケースも明確に
   test("USER_LEFT actionで存在しないIDを指定してもユーザーリストは変更されない", () => {});
   ```
...
5. **ドメイン駆動のテスト名**
   ...
   ```typescript
   // ❌ 実装詳細に依存した名前
   test("isConnectedがtrue、currentUserIdとusersが設定され、errorがクリアされる", () => {});
   
   // ✅ ビジネスロジックを表現した名前
   test("接続状態となり、エラーが初期化され、自分を含めた入室中のユーザーが設定される", () => {});
   ```

6. **技術用語からドメイン用語への置き換え**
   - リスト/配列 → コレクションの意味に応じた用語（例：チャット履歴、参加者一覧）
   - 追加/削除 → ビジネスアクション（例：参加する、退出する）
   - フィールド/プロパティ → ビジネス上の意味（例：接続状態、エラー情報）
